...
function MenuAutoEcole()
    local MAutoEcole = RageUI.CreateMenu("~u~Menu touche", "Pour savoir les touches du serveur")
    MAutoEcole:SetRectangleBanner(175, 175, 175)
    ...
    while MAutoEcole do
        Wait(0)
            ...
            RageUI.ButtonWithStyle("~m~Anonyme",nil, {RightLabel = "~m~/ooc"}, true, function(Hovered, Active, Selected)
                if Selected then       
                    local ooc = MenuToucheKeyboardInput("Entrer votre texte anonyme", "", 30)
                    if ooc then
                        ExecuteCommand("ooc "..ooc)
                        RageUI.CloseAll()	
                    end	
            end
        end)
        ...
        RageUI.Separator("~y~Bonne game à vous!")
              ...
              if not RageUI.Visible(MAutoEcole) then
